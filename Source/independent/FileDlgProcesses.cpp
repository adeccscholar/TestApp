...
void TFileDlgProcess::SplitPath(TMyForm& frm) {
   ...
   try {
      ...
      if(auto theFile = frm.Get<EMyFrameworkType::edit, string>("edtPath"); theFile) {
         if(fs::path check = *theFile; check.root_name() != fs::path("")) {
            strDirectory = frm.Get<EMyFrameworkType::edit, string>("edtFile");
            frm.Set<EMyFrameworkType::edit>("edtFile", "");
            frm.Set<EMyFrameworkType::edit>("edtPath", strDirectory);
            }
         else {
